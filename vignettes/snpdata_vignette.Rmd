...
gaf="/Users/km28/Documents/Karim/Packages/Pf_gene_ontology.txt"
gff="/Users/km28/Documents/Karim/Packages/PlasmoDB-56_Pfalciparum3D7.gff"
```
...
4. the gene ontology annotation file (optional). It can be downloaded from https://plasmodb.org/plasmo/app/downloads/Current_Release/Pfalciparum3D7/gaf/   
5. the gene annotation file (optional). It can be obtained from https://plasmodb.org/plasmo/app/downloads/Current_Release/Pfalciparum3D7/gff/    
If you don't have the `gaf` and `gff` file, these can be set at `NULL` or ignored. In that case, the gene annotation and description column will be missing from the `snpdata$details` data frame.   
type `?get_snpdata` for more details      
```{r eval=FALSE}
snpdata=get_snpdata(vcf.file = vcf, meta.file = metadata, output.dir = outDir, gaf = gaf, gff = gff)
```
